...
const BANDS = ['rise', 'grow'];   // 흰 띠 효과: 스윽 올라오기 / 천천히 확대
(async () => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const bm of BANDS) for (const m of MOTIONS) {
      const r = await page.evaluate(async (m, bm) => {
        const wait = ms => new Promise(res => setTimeout(res, ms));
        document.querySelector(`[data-hook-motion="${m}"]`).click();
        document.querySelector(`[data-hook-band-motion="${bm}"]`).click();
        await wait(40);
        ...
        const camZoom = () => { const c = document.querySelector('.scene-camera'); return c ? (new DOMMatrix(getComputedStyle(c).transform).a || 1) : 1; };
        const at = t => { window.sceneStyle.motionAt(t); const a = band.getBoundingClientRect(), b = box.getBoundingClientRect();
          return [a.top, b.top, +getComputedStyle(band).opacity, +getComputedStyle(box).opacity, a.width, b.width, a.top + a.height / 2, b.top + b.height / 2, camZoom()]; };
        ...
        if (bm === 'grow') {
          // 확대: 글자·상자가 같은 배율로 커지고 가운데가 어긋나지 않는지(카메라 배율은 나눠서 본다)
          // 화면 배율(카메라)을 두 시점 모두 나눠야 한다 — 떨림은 처음부터 1.03배라 끝만 나누면 1.12/1.03으로 오판(실측)
          const tScale = (e[4] / e[8]) / (s[4] / s[8]), bScale = (e[5] / e[8]) / (s[5] / s[8]);
          return {bm, tScale: +tScale.toFixed(3), bScale: +bScale.toFixed(3), centerGap: +Math.abs((e[6] - e[7]) / e[8] - (s[6] - s[7]) / s[8]).toFixed(1),
                  op: [s[2], e[2], s[3], e[3]].map(v => v.toFixed(2)).join('/'),
                  textInside: (() => { const rg = document.createRange(); rg.selectNodeContents(band); const q = rg.getBoundingClientRect(), f = document.querySelector('#a-live-preview').getBoundingClientRect();
                    return q.left >= f.left - 1 && q.right <= f.right + 1; })()};
        }
        return {bm, textMove: Math.round(s[0] - e[0]), boxMove: Math.round(s[1] - e[1]), gapDrift: +(gapS - gapE).toFixed(1), zoom: +zoom.toFixed(3), op: [s[2], e[2], s[3], e[3]].map(v => v.toFixed(2)).join('/'),
  ...
  console.log(fail ? `실패 ${fail}건` : `전부 통과 (${count}종 × ${MOTIONS.length}모션 × 흰 띠 ${BANDS.length}종)`);
  await browser.close();
