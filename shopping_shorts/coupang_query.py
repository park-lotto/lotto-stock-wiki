...
- ★우선순위: 대본 > 캡션 > 분석 키워드 > 썸네일 판독 힌트. 힌트는 **화면에 찍힌 물건**일 뿐이라
  글이 가리키는 물건과 다르면 버려라(2026-09-26 실사고: 캡션 "간장 대신 이 마법의 비밀 소스 한 스푼"인데
  힌트 "스텐 후라이팬"을 답함 — 정답은 그 소스다. 조리에 쓰인 팬·냄비·그릇은 글이 그것을 팔지 않는 한 답이 아니다).
- 글이 물건을 일부러 숨겼고("이 소스", "이것 하나") 대본·캡션 어디에도 이름이 없으면, 힌트로 대신 채우지 말고
  product를 빈 문자열로 — 그래야 화면이 "대본으로 다시 찾기"를 안내한다.
