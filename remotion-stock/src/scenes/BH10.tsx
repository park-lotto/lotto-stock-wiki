...
export const BH10 = () => {
  ...
  const pulseMag   = Math.min(1, Math.max(0, (f - 88) / 40));
  ...
  // 채널명 N3 글로우 (solid color)
  const pulse2  = Math.sin(f * 0.07) * 0.5 + 0.5;
  const gSz2    = interpolate(pulse2, [0, 1], [10, 34]);
  const nameGlow = t1Op > 0.5 ? `0 0 ${gSz2}px #00FFD0, 0 0 ${gSz2 * 2}px rgba(0,255,208,0.5)` : undefined;
    ...
    <AbsoluteFill style={{ background: C.bg, opacity: fadeIn, fontFamily: FONT }}>
      ...
      {PARTICLES.map((p, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            background: C.main, opacity: op * 0.55,
      ...
      <div style={{
        ...
        <div style={{
          ...
          letterSpacing: t1Ls,
          color: C.main, textShadow: nameGlow,
          lineHeight: 1.15,
        }}>로또의 주식</div>
        ...
        <div style={{
          ...
          background: C.main, borderRadius: 16, padding: '22px 72px',
