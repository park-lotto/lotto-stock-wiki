...
(async()=>{
  ...
  const measure=()=>page.evaluate(()=>{
    ...
    return {
      박스:{좌:pv.left+box.x*sx,우:pv.left+(box.x+box.width)*sx,
            위:pv.top+box.y*sy,아래:pv.top+(box.y+box.height)*sy},
      글자:{좌:t.left,우:t.right,위:t.top,아래:t.bottom},
      ...
      for(const sc of (mode==='story'?[0,1]:[0])){
        ...
        const report=(tag,j)=>{ if(!j)return;
          for(const [k,label] of [['좌','왼쪽'],['우','오른쪽'],['위','위'],['아래','아래']])
            if(j[k]>TOL)fails.push(`${where} ${tag} → 채널명이 상자 ${label}으로 ${j[k]}px 넘침(글꼴 ${j.글꼴})`);
        };
        report('기본',base);
