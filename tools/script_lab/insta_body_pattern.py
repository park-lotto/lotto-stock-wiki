...
AXES = [
    ...
    ('리스트형',    r'(\d+\s*개|다섯|일곱|열\s*개|세\s*가지|알려드릴|모아왔|꿀템)'),
    ('지목호명',    r'(하시는\s*분|계시면|계신\s*분|둔\s*엄마|쓰시는\s*분|포기했던\s*분|사는\s*분)'),
    ('공감질문',    r'(죠\?|많죠|기억나세요|아니냐며|하시죠|안\s*지워지|되네)'),
    ('금지경고',    r'(마세요|마시고|하지\s*마|주지\s*마|안\s*돼요|당장\s*보세요|큰일)'),
    ('무지후회',    r'(몰라서|모르면|나만\s*몰랐|진작|할\s*뻔|바꿀\s*뻔)'),
    ('지인증언',    r'(시어머니|시부모|어머님|시누이|친구네|친구집|친구가|동창|와이프|남편|언니네|언니를|엄마한테|엄마가|딸이|아들이|애\s*친구|선생님한테|사장님이)'),
    ('충격선언',    r'(충격|소리\s*질렀|난리|헛웃음|웃음부터|기절|깜짝\s*놀랐|역대급|발견했|대박)'),
    ('해외발견',    r'(일본|중국|미국|독일|교도소|라쿠텐|해외|유럽)'),
    ('감탄개시',    r'^\s*(와|우와|으아|헐|아니)\b|^\s*(와|우와|으아)[,\s]'),
