...
- ★★★훅은 은행에서 가져와라(하드코딩 반복 금지): 아래 [은행] 블록에 '훅' 부품이 있으면 반드시
  거기서 **매번 다른 것**을 골라 이 영상 소재로 변형해 써라 — 은행엔 매일 좋은 훅이 수북이 쌓인다.
  그게 은행을 두는 이유다. 은행 훅이 없을 때만 아래 유형(충격/감탄·뒤늦은 발견·극찬·강한 경고·
