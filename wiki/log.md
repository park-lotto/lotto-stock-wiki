- 2026-07-10 — [구현·배포] **쇼핑쇼츠 방향수정(구매처찾기 제거)+5개어 정확도개선+샤오홍슈+속도개선**(`shopping_shorts/`): 전날 만든 "구매처 찾기"(SerpApi Lens 쇼핑몰검색)가 목적 오해(짜집기용 타영상 찾기가 진짜 목적)였음을 사용자가 정정 — 전체 제거. "정확도 떨어짐" 원인 발견: Gemini가 ko/en/zh 3개어 키워드 생성하는데 검색은 언어 하나만 써서 zh는 버려지고 있었음 → ko/en/zh/ja/ru 5개어 확장+전부검색+URL중복제거. "5대플랫폼" 요청으로 샤오홍슈(zen-studio~rednote-search-scraper) 실수집 신규 추가(더우인 액터는 흔한키워드도 0건 나와 신뢰도낮아 보류). 5개언어 순차검색+후보별 순차Gemini채점이 실측 27분+ 걸리던 것 발견 → ThreadPoolExecutor 병렬화로 3분47초로 단축(약7배). 전부 커밋+배포+라이브검증(119 유닛테스트). 상세=NEXT_SESSION.md.
