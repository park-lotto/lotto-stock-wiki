...
def test_씨앗_선택이_소재_확인_기준임을_화면에_안내한다():
    src = _produce_js()
    assert "대본 생성 시 씨앗 영상을 하나 고르세요" in src
    assert "대본의 소재를 확인할 때 사용됩니다" in src


def test_픽업은_스타일_선택과_배타가_아니다():
    """사장님: "원래 대본 두개선택인데 픽업영상대본 선택후 다른거 누르면 1개밖에 안되는데?"

    ★원인은 버그가 아니라 설계였다 — 픽업이 `S2.picked.length===0`일 때만 켜지는
      **배타 구조**라, 스타일을 하나 고르면 픽업이 자동으로 꺼졌다(픽업 = '스타일 0개').
    → 별도 플래그(S2.usePickup)로 떼어내 스타일 선택과 독립시켰다.

    ⚠️2026-09-20 픽업영상 대본 **카드는 폐지**됐다(씨앗 유형 자동 선택으로 갈음).
      그래서 옛 판정("📌 픽업영상 대본" 카드를 찾아 clearStyles가 없는지")은 더 못 쓴다
      — 그 문구를 찾다 ValueError로 죽었다(2026-09-21 게이트 실패의 원인).
      지킬 것은 **배타 구조로 돌아가지 않는 것**이므로 플래그 자체를 본다."""
    src = _produce_js()
    assert "S2.usePickup" in src, "픽업이 여전히 picked.length로 판정된다(배타 구조)"
    # 플래그를 picked 길이로 되돌리면 그때가 배타 구조 복귀다.
    assert "S2.usePickup = (S2.picked" not in src.replace(" ", " "),         "usePickup을 picked 길이로 정하면 배타 구조로 되돌아간다"
