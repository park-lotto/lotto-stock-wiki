  ...
  const report=await page.evaluate(async()=>{
    ...
    for(let i=0;i<rows.length;i++){
      ...
      for(const line of rows[i].frame.lines.filter(line=>line.bind!=='caption')){
        ...
        const wantTitle=(window.PRESET_FONTS||{})[rows[i].id]?.title||line.font_family;   // 09-19: 템플릿별 기본 글꼴이 생겨 데이터 글꼴 대신 이 값과 대조한다
      ...
      const wantCap=(window.PRESET_FONTS||{})[rows[i].id]?.caption||'PretendardXBold';
