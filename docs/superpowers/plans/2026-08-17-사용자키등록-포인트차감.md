...
`shopping_shorts/app.py` — **`_STATIC` 정의(`app.py:10192`) 다음에** 추가한다.
⚠️ 그 위에 넣지 마라. 모듈 로드 시점엔 함수 본문이 실행되지 않아 당장은 돌지만,
`_STATIC` 정의 위에 있는 코드는 읽는 사람을 헷갈리게 한다. 정의 아래에 둔다.

먼저 실제 정의 위치를 확인한다:

```bash
grep -n "_STATIC = " shopping_shorts/app.py
```

Expected: `10192:_STATIC = Path(__file__).parent / "static"`

그 아래(다른 `page_*` 라우트들과 같은 자리)에 추가:
...
`shopping_shorts/app.py:6778` 근처:
...
`shopping_shorts/app.py:4857` — `check_and_count(cid, "lens")` **직후**에 추가:

```python
    # 포인트 차감(2026-08-17) — 일일 상한과 별개다. 내 키를 쓰면 안 깎인다.
    from shopping_shorts import keyroute as _kr, points as _pt, pricing as _pr
    if _kr.should_charge(store, cid, "gemini"):
        _need = _pr.cost(store, "lens")
        if not _pt.deduct(store, cid, _need, "lens"):
            return JSONResponse(status_code=402, content={
                "ok": False, "error": f"포인트가 부족합니다 (필요 {_pr.to_display(_need)}P)"})
```

- [ ] **Step 5: 대본 차감 배선**

`app.py:1694`·`app.py:1871`·`app.py:8195` 세 곳의 `check_and_count(..., "script")` 직후에
같은 패턴을 넣되 op를 `"script"`로 한다.

- [ ] **Step 6: 영상제작 차감 배선**

`app.py:2675` `check_and_count(cid, "render")` 직후에 op `"mix"`로 같은 패턴.
