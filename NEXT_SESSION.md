...
- lens_discover(google_lens 5플랫폼 도메인필터)/store 월카운트/api_lens_search+api_media/유튜브·틱톡 mp4재생통일(embed폴백)/렌즈탭UI(canvas캡처+토글칩5개+결과카드). 월 100회 가드(SerpApi 무료). python-multipart 서버설치 완료.
- ⏭ **남은 것(다음 세션)**: 라이브 대시보드 로그인 → 인스타 카드 재생→멈춤→🔍렌즈 → **실제 SerpApi 결과 육안검증**(중국플랫폼 실제 나오는지 관찰). 유튜브/틱톡 카드 mp4재생 여부(실패시 embed폴백)도 확인. / 향후: 렌즈도 포인트=현금 차감.
...
### ⏭ 남은 일 (배포 후속)
1. **A4 원격 크론 등록**: 서버 배포 반영 후 SSH로 `crontab -e`에 계획서 Step7의 줄 추가 — `0 4 * * * cd /home/ubuntu/lotto-stock-wiki && /home/ubuntu/venv/bin/python -m shopping_shorts.daily_batch >> /tmp/shopping_shorts_daily_batch.log 2>&1`. 등록 후 수동 1회 실행 검증.
2. **UI 실브라우저 검증**(미완): 서버 라이브에서 library 카드 🎬 → 요소 드롭다운(랜덤/카테고리/자유즉흥)·초안 텍스트박스 편집→자동저장·지시문 재작성·📜버전이력. (JS파싱·핸들러·엔드포인트·백엔드 E2E는 확인됨, 실렌더만 남음.)
3. (선택) A7 마이너 하드닝 4건: `n=int()` 파싱 안전·`option value` 라벨 esc·카테고리 DOM추출 견고화·elem_modes non-dict 가드.
