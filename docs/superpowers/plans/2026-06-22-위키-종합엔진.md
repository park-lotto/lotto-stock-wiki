...
**Tech Stack:** Python 3.11, SQLite, `anthropic` SDK 0.104.1 (claude-sonnet-4-6), `google-genai`(Gemini, 기존), pytest.
...
- Sonnet 모델 ID: `claude-sonnet-4-6`. 비용절감: 변동 종목만 호출.
...
- [ ] **Step 1: ANTHROPIC_API_KEY 확인**

Run: `python -c "import os; print('OK' if os.environ.get('ANTHROPIC_API_KEY') else 'MISSING')"`
Expected: OK (없으면 `.env`/환경변수 설정)
