...
# ⚠️2026-09-18부로 **아무도 안 쓴다**(죽은 상수). `density_target`에서 뺐다 —
#   목표까지 끌어올리면 느린 스타일이 전부 바닥에 붙어 스타일별 밀도가 죽는다(0순위-B:
#   시간 강제는 아래 `_LEN_FLOOR` 한 곳에서만). 지우지 않고 남기는 이유는 09-09의
#   취지("25초에 9.6초 대본 금지")가 어디로 갔는지 다음 사람이 추적할 수 있게 하기 위함.
_FILL_FLOOR = 0.8

# 허용 **하한**을 목표 초의 몇 할로 잡을지. 0.7 = 25초짜리면 17.5초 미만은 반려.
# ★시간 강제(09-09 취지)를 지키는 **유일한** 자리다 — `density_range`가 이것으로 반려선을
#   정한다. 목표(`density_target`)는 스타일의 색만 말하고, 반려는 여기서만 한다.
_LEN_FLOOR = 0.7
...
def density_target(style, seconds=30):
    ...
    # ★2026-09-18: 바닥을 **목표에서 뺀다**. 시간 강제는 `density_range`의 `_LEN_FLOOR`가
    #   이미 한다(반려선) — 여기서 또 올리면 **같은 판단이 두 곳**이 되고(0순위-B),
    #   느린 스타일의 목표가 바닥까지 끌려 올라가 **스타일별 밀도가 통째로 죽는다.**
    #   실측(2026-09-18, 25초): 바닥이 목표에 있으면 밀도 240·264·300이 **전부 232**로
    #   같아진다 — 08-24가 되살린 그 병(스타일 11개가 한 창을 쓰던 것)이 그대로 재발한다.
    #   `test_density_target_respects_styles_below_the_cap`이 이것을 지키는 계약이다.
    #   ⚠️바닥을 없앤 게 아니다. 09-09이 잡으려던 "25초에 9.6초 대본"은 `density_range`의
    #     `lo = max(tgt*DENSITY_LO, _LEN_FLOOR*cap)`가 계속 막는다 — 거기가 유일한 반려선이다.
...
def density_range(style, seconds=30):
    ...
    hi = min(int(tgt * DENSITY_HI), cap)
    # ★창이 뒤집히면 그 스타일은 **무엇을 써도 반려**다(2026-09-18 실측).
    #   느린 스타일은 hi(=tgt*1.4)가 길이바닥(lo)보다 작아진다:
    #     raw 102 → tgt 62 → hi 86 인데 lo 169  →  169~86 = 통과 불가능한 창.
    #   종전엔 lo만 cap으로 조이고 hi와 **대조하지 않아** 이 모양이 그대로 나갔다.
    #   ★고치는 방향은 **hi를 올리는 것**이지 lo를 내리는 것이 아니다:
    #     - lo를 내리면 09-09의 길이바닥이 무너진다(`느린_스타일도_바닥_아래로는_안_내려간다`).
    #       바닥은 "시킨 초를 채워라"는 약속이라 스타일이 느리다고 깎아주면 안 된다.
    #     - hi는 "그 스타일치고 과한가"를 보는 **상대 기준**이라, 바닥이 이긴 구간에서는
    #       기준으로서 의미가 없다. 바닥 위로 밀어 창을 연다.
    #   ⚠️cap(플랫폼 규격)은 절대 안 넘는다 — 그게 08-18이 박은 진짜 천장이다.
    hi = min(max(hi, lo), cap)
    return min(lo, cap), hi
