...
def _scene_first_candidates(inventory_text, reference_text, target_seconds, n=3, call=_vault_call,
    ...
    prompt = (
        ...
        # 막고 구성은 맡긴다.
        "→ 스펙을 그대로 나열하지 말고, 이걸 겪은 사람의 이야기로 바꿔 써라.\n\n"
        ...
        + "[이야기]\n"
        "- 기능 소개가 아니라 한 사람이 겪은 짧은 이야기다. 누가·무슨 일이 있었고·그래서 어떻게 "
        "됐는지가 하나로 이어지게 써라. 문장이 각자 놀면 광고 문구로 들린다.\n"
        "- 훅·이야기·CTA는 **후보마다 개성 있게** 달라야 한다. 같은 인물·같은 사건을 "
        f"{n}개가 돌려쓰지 마라 — 각 후보는 다른 각도에서 출발한 다른 이야기다.\n"
