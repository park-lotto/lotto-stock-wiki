...
def _backbone_script_prompt(flow, inventory, target_seconds, style_block=""):
    ...
    return (
        ...
        "화면=행위, 대사=이야기.\n"
        "★★★스토리 아크를 비트에 분배해 하나의 이야기로 이어라(이게 스토리짤의 핵심):\n"
        "  - 앞 1~2비트 = 인물+상황 설정(예: '남편이 아침마다 대충 때우길래 속상했거든요').\n"
        "  - 중간 비트 = ★그 인물 이야기를 이어가라 — 세운 인물(남편·아이 등)을 중간에 버리고 "
        "요리설명('우유를 섞으세요')으로 새면 실패다. 만드는 과정 위에 그 사람의 기대·긴장·비법을 "
        "얹어라(예: '이거 하나면 남편이 깜짝 놀랄 거예요', '근데 여기서 대충 하면 남편이 또 남기죠').\n"
        "  - 끝 비트 = 그 인물의 반응·결과로 닫아라(예: '남편이 이것만 찾네요') + CTA.\n"
        "즉 앞에서 세운 인물이 중간·끝까지 관통해야 한다. 좋은 대사 나열이 아니라 '한 사람의 이야기'다.\n"
