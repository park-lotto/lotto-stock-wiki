...
Task 1이 남긴 fixture를 열어 노드가 어느 경로에 들어있는지 **눈으로 확인한 뒤** `extract_post_nodes`를 쓴다. 인스타는 `data.xdt_api__v1__clips__user__connection_v2.edges[].node.media` 였다 — 쓰레드는 다르다. 확인 명령:

```bash
py -X utf8 -c "
import json;d=json.load(open('shopping_shorts/tests/fixtures/threads_profile_payload.json',encoding='utf-8'))
for x in d:
    if 'like_count' not in x['body']: continue
    o=json.loads(x['body']); 
    def walk(n,p=''):
        if isinstance(n,dict):
            if 'code' in n and 'like_count' in n: print('NODE PATH:',p); return True
            for k,v in n.items():
                if walk(v,p+'.'+k): return True
...
확인된 경로로 `extract_post_nodes`를 쓰고, fixture를 읽는 테스트를 한 개 추가한다:

```python
def test_실페이로드에서_노드가_한_개_이상_나온다():
    import json, pathlib
    from shopping_shorts.threads_parse import extract_post_nodes
    ...
    for x in raw:
        try:
            nodes += extract_post_nodes(json.loads(x["body"]))
        except ValueError:
            pass
    assert len(nodes) > 0
```
...
def _fetch_profile_nodes(username, session_path="", proxy=""):
    """프로필을 열고 GraphQL 응답에서 게시물 노드를 모은다(순서 보존)."""
    import json
    nodes = []
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        ...
        def on_response(res):
            if "graphql" not in res.url:
                return
            try:
                nodes.extend(extract_post_nodes(json.loads(res.text())))
            except Exception:
                pass          # 우리가 모르는 모양은 건너뛴다(성패는 건수로 본다)
