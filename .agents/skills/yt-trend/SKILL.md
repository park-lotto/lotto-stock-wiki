---
name: yt-trend
description: "유튜브 제작가이드2 — 당일 급상승 키워드 기반 영상 소재 발굴 → 대본 초안 자동 생성 파이프라인. 5단계: YouTube Top20 수집(당일 12시간) → 실시간 시황 반영 Gemini 딥리서치 → 자막 분석 → 소재 추출 → 대본 초안 S1~S8"
metadata:
  tags: youtube, trend, 소재발굴, 파이프라인, gemini, 유튜브제작가이드2
---
