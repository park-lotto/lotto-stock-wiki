...
def client(tmp_path, monkeypatch):
    ...
    관리자 라우트 검사는 DASH_PASS를 켜는 별도 테스트에서 한다."""
...
★**`DASH_PASS`를 반드시 세팅해야 한다**(2026-08-25 실측). 안 하면 이 코드베이스는
**인증을 통째로 끈다**(기동 시 경고: "DASH_PASS 미설정 → 인증 OFF(전원 full/admin)").
실측: `DASH_PASS` 없음 → `/api/admin/customers`가 **200**, 세팅 → **401**.
이걸 모르고 짜면 가드가 없어도 통과하는 **가짜 green**이 된다.

```python
def test_admin_routes_require_admin(tmp_path, monkeypatch):
    """검수 화면은 관리자만 — 초안을 아무나 승인하면 안 된다.

    ★DASH_PASS를 세팅해야 인증이 켜진다(미설정이면 이 코드베이스는 전원 admin 취급).
      안 켜고 재면 가드가 없어도 통과하는 가짜 green이 된다."""
    monkeypatch.setenv("DASH_PASS", "x")
    from fastapi.testclient import TestClient
    from shopping_shorts import app as app_module
    monkeypatch.setattr(app_module, "DB_PATH", str(tmp_path / "t.db"))
