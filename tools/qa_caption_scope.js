...
(async()=>{const b=await pptr.launch({headless:true});try{
 ...
 await p.click('[data-caption-scope="all"]');await drag();s=await p.evaluate(()=>sceneStyle.snapshot());assert.equal(Object.keys(s.captionDrags).length,12);assert.equal(new Set(Object.values(s.captionDrags).map(v=>JSON.stringify(v))).size,1);
 const other=s.captionDrags['t11:story:2:caption'];await p.click('[data-caption-scope="scene"]');await drag();s=await p.evaluate(()=>sceneStyle.snapshot());assert.deepEqual(s.captionDrags['t11:story:2:caption'],other);assert.notDeepEqual(s.captionDrags['t11:story:1:caption'],other);
 ...
 assert.equal(await p.$eval('.scene-line-editor',e=>e.open&&!e.hidden),true);assert.equal(await p.$$eval('[data-line-inputs] input',e=>e.length),2);
 await p.$eval('[data-line-inputs] input',e=>{e.focus();e.setSelectionRange(5,5)});await p.keyboard.press('Enter');assert.equal(await p.$$eval('[data-line-inputs] input',e=>e.length),3);await p.keyboard.press('Backspace');assert.equal(await p.$$eval('[data-line-inputs] input',e=>e.length),2);
