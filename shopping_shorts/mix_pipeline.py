...
def render_inputs_for(store, job, job_id, work, keys, customer_id=0, *, allow_clean=True):
    ...
    paths = _cb.source_paths(base)
    # ★고른 장면만 지운 정본이면 안 고른 비트는 **원본 재료 그대로** 남는다(clean_base.remap_plan) —
    #   원본 소스도 같이 넘겨야 렌더·캡컷·꾸미기 프레임이 그 칸을 찾는다(없으면 청소본 엉뚱한 자리).
    _need = {m.get("video_id") for b in plan2.get("beats") or [] for m in _beat_materials(b)}
    if _need - set(paths):
        print("[clean-base] 안 고른 장면은 원본 재료로: %s" % sorted(str(v) for v in _need - set(paths)),
              file=sys.stderr)
        paths = dict(_resolve_sources(job, work))
        paths.update(_cb.source_paths(base))
