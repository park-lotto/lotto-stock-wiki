    ...
    "properties": {
        ...
        "source_brief": {
            ...
            "properties": {
                ...
                "flow": {"type": "string"},      # 시간 흐름(전환점 2~5개) — 2026-09-04
                "confidence": {"type": "string"},  # 스토리 확신도 높음/낮음/불명
            },
        },
    },
...
# ★브리프·세그 필드 정의는 **여기 한 곳**이다(0순위-B, 2026-09-04). 통째 업로드 추출(_PROMPT)과
#   컷별 프레임 태깅(frame_script)이 같은 문장을 쓴다 — 한쪽만 고치면 두 경로의 태그가 갈라진다.
_BRIEF_GUIDE = """[source_brief — 이 영상이 무슨 영상인지 먼저 한 덩어리로 정리한다]
...
- flow: 영상의 **시간 흐름**을 전환점 2~5개로(한국어 한 줄, 예: "0~4초 문제 상황 → 4~15초 제품 사용 → 15초~ 결과 확인"). 대사가 없어도 화면 순서로 잡아라.
- confidence: 위 스토리를 얼마나 확신하나 — "높음"/"낮음"/"불명" 중 하나. 화면만으론 전개를 모르겠으면 "불명"."""

_SEG_FIELD_GUIDE = """- scene_desc: 그 구간 화면에 무엇이 보이는지 짧게(제품/행동/구도). 화면 속 **주 대상을
...
_BRIEF_KEYS = ("product", "role", "core", "summary", "flow", "confidence")
_BRIEF_MAX = {"product": 40, "role": 60, "core": 80, "summary": 400,
              # ★flow·confidence(2026-09-04): 컷별 프레임 태깅(frame_script)이 2차 묘사에 쓰는 스토리 맥락.
              #   "먼지 낀 방충망"이 before인지 그냥 방충망인지는 전체 흐름을 알아야 정해진다.
              "flow": 200, "confidence": 10}
