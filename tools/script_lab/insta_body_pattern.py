...
#
# ★안 넣은 축(4차 발굴에서 보였지만 승격기준 8편+/3채널+ 미달):
#     위험경고("진짜 위험해요") 2편/2채널 · 반려동물 5편/4채널
#   눈에 보인다고 넣지 않는다 — 근거 없는 축이 죽은 스파인 31개를 만들었다.
AXES = [
    ...
    # ★4차 발굴로 추가(2026-08-20) — 레시피 권유가 통째로 새고 있었다.
    #   "감자는 이렇게 드셔보세요" · "타코는 무조건 만들어 드세요" · "새송이는 제발 이렇게 드세요"
    #   기존 '무조건지시'는 r'무조건\s*(이렇게|이거|사|담)'이라 "무조건 만들어"를 못 잡고,
    #   '금지경고'는 '마세요'만 봐서 '드세요'를 못 잡았다. 실측 23편/12채널.
    #   ★이 축은 spine 53(단정 명령형)과 같은 축이다 — 새 행을 만들지 말고 53의 근거로 쓴다
    #     (16편 → 39편). 다른 축으로 새는 걸 막는 게 목적이지 새 스파인을 늘리려는 게 아니다.
    ('권유지시',    r'(이렇게|꼭|무조건|제발)\s*(드세요|드셔|드셔보세요|먹어|먹어보세요|만들어|해\s*드세요|해보세요|주세요|챙기세요|가져가세요)'),
