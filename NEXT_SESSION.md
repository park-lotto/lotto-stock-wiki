...
### ⚠️ 검증 중 새로 발견한 이슈 (미조치, 다음 세션 판단 필요)
1. **`script_extracts.category`가 항상 NULL** — 라이브 DB 확인: 13건 전부 category NULL. 신규 추출 시 `app.py`가 `item.get("category")`를 넘기긴 하지만(정상 경로), **기존/구버전 방식으로 들어온 행에 대한 일회성 category 백필 로직이 없음**. 그 결과 `daily_batch.backfill_structures()`가 `category IS NOT NULL` 조건에 걸려 **매일 0건**만 처리 중(로그로 확인). `element_category_stats`가 이미 30행 있는 건 `script_wiki`(카테고리 있음) 경로로만 통계가 쌓인 것 — 위키에 못 들어간 나머지 대본추출들은 구조분석 학습 표본에서 계속 빠짐. 근본수정 필요시: 추출 당시 category 저장 안 됐던 과거 행을 원본 소스(reference.db의 discovery/source_analysis 등)에서 역매핑해 백필하는 1회성 스크립트 필요.
2. **"AI 자유즉흥" 모드가 UI에 노출 안 됨** — 백엔드(`script_generate.py`)는 `free` 모드를 완전 지원(옵션 없을 때 자동 폴백으로 사용)하지만, `library.html`의 드롭다운 생성 코드(`openGen()`)가 `random`+`category:*` 옵션만 채워 넣고 **"자유즉흥"을 사용자가 명시적으로 고를 select option을 안 만듦**. 스펙상 4단 모드 중 하나인데 실질 3단(원본유지/카테고리지정/카테고리중랜덤)만 노출 중.
