...
## 13. 2026-09-24 CH PC — 로컬 카피 폴더 정리 (서버 호출 없음)

- 사장님 방침: 볼케이노 서버 호출(setup·프리셋·카드뉴스 살펴보기)은 **Desktop/볼케이노작업 에서 일반 창으로**. 이 창(개발 폴더)은 로컬 카피만.
- `Desktop/볼케이노작업/카피/` 생성 — runner 3버전(v1 09-11 dcfc113b / v2 09-12 58e3975d / v3 09-13 50994f62 최신) · packs 4종 · jobs 8편 설계도(토큰·probe·slot·cache 제외, 총 ~370MB) · docs 5편. 목차는 그 폴더 README.md.
- 실측: 프리셋은 도구 설명 기준 **12개**(정치·3D쇼츠 신규) + `volcano_cardnews` 도구. 실행기엔 cardnews_*.py 5개가 이미 동봉.
- 09-13에 시작만 한 편 2개 있음: 블라인드집값(prompts 단계에서 멈춤)·최민식(source만).

### ⏭ 다음
- 볼케이노작업 창에서 `setup` → 프리셋 12개·카드뉴스 규격 살펴보기 → 다른 채널 첫 편 만들어 팩 확보
