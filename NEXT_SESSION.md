...
2. **샤오홍슈(小红书) + 도우인(더우인) 실수집 완성 — 5대 플랫폼 완료**. Apify 액터 조사: 처음 조사한 `natanielsantos~douyin-scraper`는 흔한 키워드도 0건이라 보류했으나, 샤오홍슈에 쓴 `zen-studio~rednote-search-scraper`가 잘 작동한 데 착안해 같은 개발자의 `zen-studio~douyin-search-scraper`로 재시도 → 정상 작동 확인, 연동 완료. 유튜브/틱톡/인스타/샤오홍슈/도우인 5대 플랫폼 전부 실수집 지원.
...
4. **UI 정리**: 정확도 낮은 "플랫폼별 검색 링크"(수동 키워드검색 URL 나열) 섹션 삭제, 대신 실수집 버튼+썸네일 흐름만 남김. 각 후보 카드에 **언어 배지**(🇰🇷🌐🇨🇳🇯🇵🇷🇺) 추가 — "국내 재편집물보다 해외 원본 영상이 낫다"는 피드백에 따라 자동필터 대신 사용자가 직접 판단하게 함(source_candidates 테이블에 source_lang 컬럼 추가).
5. **제품 정밀도 개선(구글 렌즈 재활용)** — 영상 컨텐츠 2종 구분 확인: ①제품 직접홍보형(지금 설계중) ②레시피/생활용품 "비밀 한스푼"형(다음에 설계). ①형은 Gemini의 일반적 키워드 추측(예: "전자노트")보다 정확한 브랜드+모델명(예: "reMarkable Paper Pro")으로 검색해야 정밀도가 높음 — 6개 프레임 전부 SerpApi Google Lens로 역검색 후 여러 프레임에 걸쳐 일관되게 나오는 제품을 Gemini가 골라 확정, 5개 언어 키워드 맨 앞에 추가(`product_identify.py`). 어제 삭제한 "구매처 찾기"와 같은 Lens API를 재사용하지만 용도가 다름(쇼핑링크 노출 → 내부 키워드 정밀도 개선). 유닛테스트 통과했으나 **실영상 end-to-end 검증은 오늘 Gemini 키 소진(13개 중 7개+)으로 다음 세션으로 이월**.

**⏳ 다음**:
1. **product_identify.py 실영상 재검증** — Gemini 키 리셋 후(또는 키 추가 후) reMarkable 릴스로 재분석해 product_name이 정확히 뽑히는지 확인
2. **타입B(레시피/생활용품형) 설계** — 특정 제품 하나가 아니라 테마/카테고리 기준(예: 가지볶음→가지요리 전반, 방충망솔→방충망청소+솔 전반)으로 다양한 영상을 모으는 별도 워크플로. 5개 언어, 한국어자막(재편집물) 배제 우선까지 요구사항 확인됨, 아직 미설계.
3. 유사도 채점(score_candidate)도 대량 후보 시 여전히 병목 가능 — 필요시 배치/캐싱 검토
4. Gemini 키 풀이 오늘처럼 하루 만에 소진되는 문제 — 키 추가 또는 사용량 모니터링 필요
