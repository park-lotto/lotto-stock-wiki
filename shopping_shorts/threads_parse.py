...
# ── 재료 문턱 (2026-08-18 실측 128건으로 산출) ────────────────────────────────
# 원래 "표본 4건으로 정하면 틀린다, 200~300건 모은 뒤 분위수로"라고 미뤄뒀던 값.
# 128건까지 모아 실제 분포를 보고 정했다. 분위수가 아니라 **점수 구성**으로 갈렸다:
#
#   문턱   남는수   그중 영상        판단
#   4점     75건    58건(77%)   ← 영상 없는 17건이 섞인다(믹스 재료가 안 된다)
#   ★5점    58건    58건(100%)  ← 영상 100%. 여기가 경계다
#   7점     35건    35건(100%)  ← 영상 63건 중 35건만 남아 너무 좁다
#
# 왜 5점이 경계인가: 영상(+3)이 있어야 5점에 닿는다. 4점은 '쿠팡(2)+캡션(2)이지만
# 영상 없음'뿐이라 수집은 돼도 재료로는 못 쓴다. 즉 5점은 임의의 눈금이 아니라
# **"영상이 있다"와 같은 뜻**이다. 계정 19개에서 통과분 전건이 영상이었다(쏠림 없음).
MATERIAL_MIN_QUALITY = 5


def is_material(post, text_level=""):
    """믹스 재료로 쓸 수 있는가 — 점수와 **영상 유무를 함께** 본다.

    ★점수만으로 판정하지 않는 이유(2026-08-18, 테스트가 잡아낸 함정):
      영상 없이도 쿠팡(2)+캡션(2)+자막없음(1) = 5점이 나온다. 지금은 수집기가
      text_level을 안 넘겨(threads_playwright.py) +1이 안 붙어 실데이터 0건이지만,
      나중에 자막 판정을 배선하는 순간 **영상 없는 글이 재료 목록에 섞인다**.
      문턱 5의 근거는 "영상이 있다"였으므로, 그 뜻을 점수에 기대지 말고 여기서 지킨다.
    """
    if not isinstance(post, dict):
        return False
    if post.get("media_kind") != "video":
        return False            # 영상이 없으면 믹스 재료가 안 된다(점수와 무관)
    return quality_score(post, text_level) >= MATERIAL_MIN_QUALITY
# ─────────────────────────────────────────────────────────────────────────────
...
def quality_score(post, text_level=""):
    ...
    이 함수는 점수만 낸다. 문턱은 MATERIAL_MIN_QUALITY(=5, 2026-08-18 실측으로 확정).
    """
    if not isinstance(post, dict):
        return 0
    score = 0
