...
def test_body_primaries_reordered_tail_anchored():
    # body(꼬리 앞)만 소스 시간순 재배치, 꼬리(마지막)는 앵커로 원래 화면 고정.
    ...
    # body(훅·전개·실용) 시간순 재배치 = 2,8,10 / 꼬리(CTA)는 원래 6 고정
    assert [b["primary"]["start"] for b in out] == [2.0, 8.0, 10.0, 6.0]


def test_segment_count_per_beat_preserved():
    beats = [_beat("a", 10, alt_starts=[1]), _beat("b", 5, alt_starts=[8]),
             _beat("cta", 3, alt_starts=[0])]
    out = _chronological_respine(beats)
...
def test_multi_video_body_grouped_by_video_then_time():
    ...
    # body(a,b,c) (video_id,start) 오름차순: A-2, A-9, B-5 / 꼬리(cta) 앵커 A-1 고정
