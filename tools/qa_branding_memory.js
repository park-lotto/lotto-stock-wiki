...
(async()=>{const b=await puppeteer.launch({headless:true});try{const p=await b.newPage();await p.setViewport({width:1800,height:1300});await p.goto('http://127.0.0.1:8767/out/scene-style-ui-showcase.html?qa=1',{waitUntil:'networkidle0'});await p.click('[data-editor-tab="effects"]');
 ...
 let saved=await p.evaluate(()=>sceneStyle.branding());assert.equal(saved.watermark.x,36);assert.equal(saved.ad.x,87);
 ...
 await p.click('[data-editor-tab="effects"]');await p.click('[data-brand="watermark"] [data-brand-reset]');assert.equal(await p.evaluate(()=>sceneStyle.branding().watermark.x),36);
