...
def narr_secs(text):
    ...
    ★2026-09-18 단위 수정 — 분자가 **raw**(공백 포함)인데 분모는 **norm** 기준이었다.
      `_SYLLABLES_PER_SEC × _speech_speed()` = 7.41자/초는 공백을 뺀 글자 기준인데
      여기서만 `len(text.strip())`으로 공백을 **세고 있었다**. 단위가 어긋나면
      배수를 아무리 맞춰도 안 맞는다(메모리 `reference_길이단위_raw_norm_혼용`:
      2026-08-24에 `script_gate`는 raw→norm으로 통일했는데 **이 함수가 빠졌다**).
      실측(2026-09-18, 완성 job 25개·비트 210개의 TTS 실길이 대조):
        계획 791초 vs 실제 608초 = **30% 과대추정**, 비트 **94%(197/210)**가 부풀었다.
        raw/norm 비율 0.7395의 역수 1.35 = 이론 과대율 35%와 일치한다.
      과대추정분은 `_fill_beat_screen_time`이 화면으로 채운다 —
      **대본과 무관한 컷이 그만큼 붙는다**(2026-09-16 "10줄 지목 → 26컷"의 뿌리와 같은 축).
      ⚠️공백만 뺀다. `script_gate.norm`은 어미 표기까지 통일해 **길이 재기엔 과하다**
        (판정용 정규화와 길이용 정규화는 목적이 다르다)."""
    return round(max(1.5, len("".join((text or "").split()))
