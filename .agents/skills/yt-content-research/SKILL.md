...
## STEP 0 — 브레인스토밍 여부 확인 ⚡ (가장 먼저)

소재 탐색 시작 전 **반드시** 아래 질문을 한다:

> "방향이 아직 없으면 브레인스토밍을 먼저 할게요.
> 이미 주제나 방향이 있으면 바로 소재 탐색 들어갑니다.
> 어떻게 할까요?"

| 사용자 응답 | 처리 |
|-----------|------|
| 방향 없음 / 브레인스토밍 원함 | `superpowers:brainstorming` 먼저 실행 → 완료 후 STEP 1 진행 |
| 방향 있음 / 바로 진행 | STEP 1로 바로 이동 |

---

## STEP 1 — 핫 이슈 스캔 (소재 탐색 전 필수 준비단계)

**목표**: 지금 + 앞으로 며칠 안에 주식시장을 움직일 이슈를 정량 데이터로 찾는다.
Claude 기억이나 느낌 기반 금지. 수치로만 판단.

### 1-A. 임박 이벤트 스캔 (D+1 ~ D+7)

앞으로 며칠 안에 터질 수 있는 이벤트 먼저 확인.

```
WebSearch: "주식 주요 일정 이번주 {날짜}"
WebSearch: "FOMC 일정 / 빅테크 실적 발표 / 한국 수출 발표 {월}"
WebSearch: "코스피 주요 이슈 {날짜} 예정"
```

출력: 날짜 | 이벤트 | 관련 섹터/종목 | 예상 영향도 표

### 1-B. 현재 관심도 정량 측정

임박 이벤트 + 현재 떠오르는 이슈 각각에 대해 아래 3가지 수치 확인.

**① 뉴스 노출량**
```
WebSearch: "{키워드} 주식 뉴스" — 결과 수 + 최신 날짜 확인
WebSearch: "{키워드} site:naver.com" — 네이버 뉴스 밀도
```

**② 관련 종목 검색량 (네이버 DataLab)**
```
Fetch MCP: https://datalab.naver.com/keyword.naver?query={키워드}
→ 최근 7일 검색 트렌드 수치 확인
```
...
| 온도 | 기준 | 처리 |
|------|------|------|
| 🔴 급등 중 | 오늘/어제 뉴스 다수 + 검색량 급증 + 커뮤니티 활발 | 지금 당장 소재 확정 |
| 🟠 상승 중 | 2~4일 내 이슈 + 검색량 유지 | 각도 잡고 D+1~2 업로드 목표 |
| 🟡 예열 중 | D+3~7 임박 이벤트 있음 | 미리 준비, 이벤트 당일 업로드 |
| ⚫ 식었음 | 1주일+ 뉴스 없음 + 검색량 하락 | 폐기 → 새 이슈 탐색 |

🔴🟠만 다음 단계 진행. 🟡는 제작 예약. ⚫는 폐기.
