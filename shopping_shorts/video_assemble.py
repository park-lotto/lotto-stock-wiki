...
# 레퍼런스(바이럴 숏폼) 자막 리듬: 한 구절이 2~3어절(예: "여러분, 오이 절대",
# "냉장 보관하지 마세요", "꼭 두 세개씩")로 짧고, 무자막 구간 없이 빠르게 순차 전환.
# 레퍼런스 실측: 공백제외 평균 6.3자·최대 9자, 어절 평균 2.2·최대 3.
# → 목표를 9자로 잡아 "며칠 안 됐는데"(6자)가 한 덩어리로 붙게 한다(5자면 갈라짐).
_CAP_TARGET = 9         # 한 구절 목표 글자수(공백 제외). 레퍼런스 최대치에 맞춤.
_CAP_MAX_WORDS = 3      # 한 구절 최대 어절 수(글자수가 짧아도 3어절 넘기지 않음).
# 구절이 이 단어들로 "끝나면" 어색하다(뒤 명사를 꾸미는 관형어·부사·의존명사).
# 예) "며칠 안"|"됐는데" → "안"에서 끊지 말고 뒤와 붙인다. "이것 한"|"스푼이면" 등.
_CAP_NO_TAIL = {"한", "그", "이", "저", "안", "못", "더", "덜", "좀", "꼭", "막",
                "두", "세", "네", "몇", "각", "매", "총", "약간", "가장", "제일",
                "이런", "저런", "그런", "무슨", "어떤", "온갖", "단", "딱"}
...
def _caption_segments(narration):
    """나레이션을 **짧은 구절 단위**로 나눈다(예: "여러분, 오이 절대 / 냉장 보관하지 마세요").
    어절(띄어쓰기) 기준으로 묶되, 누적 글자수(공백 제외)가 _CAP_TARGET을 넘으면 끊어
    새 구절을 시작한다. 레퍼런스(바이럴 숏폼) 실측에 맞춰 목표를 9자로 잡아 2~3어절이
    한 덩어리가 되게 한다. 각 구절은 짧은 1줄. 반환: [구절텍스트, ...].

    자연스러움 규칙: 구절이 관형어·부사(_CAP_NO_TAIL: "한","안","이","꼭","두"…)로
    끝나면 뒤 명사를 꾸미는 말이 갈라져 어색하다("며칠 안"|"됐는데"). 이럴 땐 글자수
    목표를 넘더라도 다음 어절을 붙인다(단, 어절 상한 _CAP_MAX_WORDS는 하드리밋).
    ...
    for w in narr.split():
        ...
        # 마지막 단어가 매달리는 말이면 목표를 넘더라도 붙인다(어절 상한은 지킴).
        dangling = cur[-1] in _CAP_NO_TAIL
        # 문장부호(. ? !)로 끝난 뒤엔 문장 경계이므로 우선 끊는다(매달리는 말이면 예외).
        sentence_end = cur[-1].endswith((".", "?", "!", "…")) and not dangling
        if under_cap and (room or dangling) and not sentence_end:
            cur.append(w)
        else:
            out.append(" ".join(cur))
            cur = [w]
