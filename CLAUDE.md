...
| 툴 | 금지 | 대신 |
|----|------|------|
| `WebSearch` | 검색 결과 전문 컨텍스트 유지 | 제목 + 1줄 요약만 남기고 버림 |
| `WebFetch` / 브라우저 크롤링 | 페이지 전체 텍스트 인용 | 필요한 수치·날짜·키워드만 추출 |
| `Bash` 출력 50줄 초과 | 전체 출력 그대로 유지 | "총 X줄, 핵심: ..." 1줄 요약으로 교체 |
| `Read` 긴 파일 | 전체 내용 재인용 | 파일 경로 + 관련 줄 번호만 참조 |
| `Write` / `Edit` 완료 후 | 방금 쓴 파일 내용 재확인 | 완료 확인 없이 다음 작업 진행 |

**대화 패턴 규칙:**
- 사용자 메시지 길게 재인용 금지 — 핵심 단어만
- 작업 완료 요약은 3줄 이내
- 이전 턴에서 이미 나온 내용 반복 금지

### ⚡ 컨텍스트 자동 압축 규칙

다음 조건 중 하나라도 감지되면 **즉시** `context-engineering:context-compression` 스킬을 호출한다. 사용자에게 묻지 말고 바로 실행.

| 감지 조건 | 기준 |
|----------|------|
| 감지 조건 | 기준 |
|----------|------|
| 사용자가 "컨텍스트", "토큰", "압축" 언급 | 즉시 1회 |
| 브라우저 크롤링 결과 대량 출력 후 | 해당 툴콜 직후 1회 |
| 긴 작업 단락 완료 후 (대본 완성·ingest 완료 등) | 작업 완료 직후 1회 |

> **원칙**: 세션 전체에서 최대 1~2회만. 자주 하면 앞 내용을 잃는다.
> 압축 전에 핵심 결정사항·현재 작업 상태를 반드시 메모리에 저장할 것.
...
| "리서치해줘" / `/리서치` | `yt-gemini-pipeline` (브리프 → Gemini 딥리서치 → 스토리 → 대본) |
| "대본 써줘" / `/script` | `yt-gemini-pipeline` (브리프 → Gemini → Claude 검수) |
