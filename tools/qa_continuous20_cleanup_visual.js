...
(async () => {
  ...
  const rows = await page.evaluate(()=>window.CONTINUOUS20.map(row=>({id:row.source_id,name:row.name})));
  for(let index=0;index<rows.length;index++){
    await page.click(`[data-p20="${index}"]`);
    await new Promise(resolve=>setTimeout(resolve,60));
    await (await page.$('#a-live-preview')).screenshot({path:path.join(process.env.TEMP,`cleanup-${String(index).padStart(2,'0')}-${rows[index].id}.png`)});
  }
