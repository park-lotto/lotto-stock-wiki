...
저장소의 **`크리에이티브 라이브러리 열기.cmd`**를 더블클릭한다. Python과 fontTools가
설치된 이 PC에서는 브라우저에 검색·분류·실제 폰트 견본·음성 미리듣기·상세 정보가 열린다.
바탕화면의 `크리에이티브 라이브러리` 바로가기도 같은 화면을 연다.

`py -m creative_library.viewer`로도 실행할 수 있다. 서버가 필요 없는 로컬 HTML이며,
실행할 때 파일 원장의 공용140개를 새로 읽는다. DB와 고객 개인자료는 포함하지 않는다.
폰트 견본은 실제 폰트의 샘플 글리프를 HTML 안에 담으며, 샘플 생성 실패 시 대체 폰트를
진짜 견본처럼 표시하지 않는다. 레이아웃은 구조 정보이며 실제 렌더 미리보기는 후속이다.
MOV 모션은 원본 파일 열기 링크로 제공한다. 읽기 전용 상세 정보의 승인은 여전히 미확인이다.
