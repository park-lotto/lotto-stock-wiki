...
**진행 상태**: 브레인스토밍(질문 6개 답변완료) → 설계 스펙 작성·커밋 → **구현 계획 작성·커밋까지 완료. 코드 구현은 아직 0개 태스크도 착수 안 함.**
...
### 다음에 할 일
1. **사용자에게 실행방식 확인**: A) 서브에이전트 기반(태스크마다 새 서브에이전트+검토, 추천) / B) 인라인 실행(이 세션에서 직접, 묶음 체크포인트)
2. 확정되면 계획 문서의 **Task A1부터 순서대로** 구현(A1~A7 → B1~B5). 각 태스크는 TDD(실패테스트→구현→통과→커밋) 단위라 태스크 하나 끝날 때마다 안전하게 중단 가능.
3. Task A4는 서버 배포 후 원격 SSH crontab 등록이 필요(계획 문서 Step 7에 정확한 crontab 줄 있음).
...
## 🅒 틱톡 키워드검색 발굴(Apify B2) — 2026-07-13 (사무실) → 집에서 이어서

**주제**: 쇼핑쇼츠 레퍼런스 랭킹에 틱톡 키워드검색 발굴 추가. **설계 확정·실증 완료, 코드만 남음.** 사용자가 "고" 하면 바로 구현 시작.
