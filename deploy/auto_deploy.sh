...
try:
    ...
    n = con.execute(
        ...
        # preview_status/clean_status도 본다(2026-07-19 실사고 2회: 1단계 미리보기 ffmpeg가
        # 배포 재시작 SIGTERM에 두 번 연속 사살 — 기존 가드는 최종렌더 status만 봤다).
        "SELECT COUNT(*) FROM mix_jobs "
        "WHERE (status IN ('rendering','removing_subtitles') "
        "       OR preview_status = 'rendering' OR clean_status = 'cleaning') "
