...
def _auto_register_instagram():
    """인스타 계정 발굴(해시태그 탐색) 상위 N 자동등록(킬스위치·실패격리). 등록 수 반환."""
    from shopping_shorts import config
    if not getattr(config, "INSTAGRAM_AUTO_DISCOVER", False):
        return 0
    try:
        from shopping_shorts import service
        added = service.auto_register_instagram(top_n=config.INSTAGRAM_AUTO_TOP_N)
        return len(added)
    except Exception as e:  # 발굴 실패(세션 만료·차단 등)가 배치 전체를 막지 않게
        print(f"daily_batch: 인스타 발굴 자동등록 스킵 — {e}")
        return 0
