...
### 🏷 탭 제목 = 지금 무슨 작업 중인지 (2026-07-16~)

**창을 6개 띄워도 탭만 보면 어느 트랙인지 안다.** 자동이라 할 일은 없다.

| 어디서 열었나 | 탭 제목 |
|---|---|
| `.tracks/보이스` | **보이스** |
| `.tracks/보이스/shopping_shorts` (더 깊은 곳) | **보이스** (같은 트랙) |
| main 폴더 | **main · 위키/조사** |

원래 Claude Code는 탭 제목을 **세션 이름**에서 가져오는데(설치본의 `terminalTitleFromRename`,
기본 켜짐) 그 이름은 **대화 내용 자동요약**이다. 그래서 퍼미션 얘기로 시작한 창은 탭에
"권한 없이 작업 진행하기"가 떴다 — 6창을 구분할 수 없었다.

`tools/session_title.py`가 **작업 폴더에서 트랙명을 뽑아** 제목으로 박는다. 트랙명은 추측할
필요가 없다 — 위 규칙이 "코드는 트랙 폴더에서"를 강제하므로 **폴더가 곧 트랙**이다.

- `SessionStart` → `sessionTitle`로 세션 이름 자체를 덮는다(자동요약이 처음부터 안 뜬다).
- `UserPromptSubmit` → 매 턴 `terminalSequence`(OSC 2)로 탭을 다시 박는다.
  자동요약이 나중에 세션 이름을 갈아치워도 **탭은 트랙명을 지킨다**.
- 배선: `.claude/settings.json`. 허용 이스케이프는 설치본에 박혀 있다 — **OSC 0/1/2/9/99/777 + BEL만**
  통과하므로 OSC 2를 쓴다. 다른 걸 쓰면 Claude Code가 조용히 거절한다.

> `/rename`으로 직접 붙인 이름은 매 턴 덮지 않는다(`UserPromptSubmit`은 탭만 다시 박고
> 세션 이름은 안 건드린다). 임시로 다른 이름을 쓰고 싶으면 `/rename`을 쓰면 된다.
