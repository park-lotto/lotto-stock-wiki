...
### ★"틱톡도 프록시 되잖아" — 맞다. 막는 건 IP가 아니라 **세션**이었다

사장님 지적을 받고 서버에서 직접 재봤다. 결론부터: **프록시는 잘 간다.**

| 확인한 것 | 결과 |
|---|---|
| 프록시(kr)로 검색 페이지 도달 | ✅ 200, 제목도 `TikTok에서 '키즈카메라' 검색`(한국어 정상) |
| `/api/search/general/full/` 응답 | ✅ **200** |
| 그 응답 **본문** | ❌ **비어 있음**(JSON 파싱 실패) |
| 화면 | ❌ "문제가 발생했습니다" + **로그인 모달** |
| 캡차 | 없음 |

→ 차단은 IP가 아니라 **로그인 세션**이다. 이건 이미 겪은 구도와 똑같다:

| 플랫폼 | 세션 | 결과 |
|---|---|---|
| 샤오홍슈 | **있음** | 무료 성공(pw_xiaohongshu) |
| 도우인 | 없음 | Apify 폴백 |
| **틱톡** | **없음** | Apify 폴백 ← 지금 여기 |

그래서 **도우인과 같은 모양으로 무료 경로를 미리 깔아뒀다**:
`config.TIKTOK_SESSION_PATH`(기본 `/home/ubuntu/tiktok_session.json`) +
`kw_backends.pw_tiktok`(프록시+세션) 을 사슬 **맨 앞**에 두었다.

```
_CHAIN["tiktok"] = [pw_tiktok(무료), apify_tiktok($0.0195)]
```

**세션 파일이 생기는 순간 자동으로 $0이 된다** — `_CHAIN`도 엔드포인트도 프론트도
고칠 필요가 없다. 세션이 없으면 **브라우저를 아예 안 띄우고** 즉시 0건으로 접는다
(괜히 띄우면 프록시 바이트만 버린다).

⚠️ 함수 이름을 `tiktok` → `apify_tiktok`으로 바꿨다. 비용표 `_COST` 키도 같이 바꿨다 —
   **비용은 백엔드 함수 이름으로 조회**되므로 한쪽만 바꾸면 $0으로 조용히 잘못 보고된다.

**라이브 확인**(배포 후): `틱톡 세션 존재? False` → `backend=apify_tiktok $0.0195`,
총 24건 21.2초. 폴백이 설계대로 돈다.

### 테스트

`shopping_shorts/tests/test_kw_search.py` **24개** 신규(네트워크 안 탐).
...
`/home/ubuntu/tiktok_session.json` 에 로그인 세션(Playwright storage_state)을 넣으면
그 순간부터 틱톡 검색이 **$0.0195 → $0**이 된다. 코드는 이미 다 깔려 있다.
샤오홍슈 세션(`rednote_session.json`)을 만든 것과 같은 방법이다.
⚠️ 도우인은 QR 로그인이 +82 번호 SMS 거부로 막혀 있다 — 틱톡도 같은 벽에 부딪힐 수 있다.
