...
def test_orb_labels_and_mapping():
    ...
    # ★2026-09-01 TTS음성을 자막제거 **앞으로** 옮겼다 — 자막제거가 2콜 나가던 뿌리다.
    #   자막제거는 조립된 완성본 1편을 청소하는데 컷 길이가 음성 발화초에 맞춰져 있어서,
    #   청소 뒤에 음성을 고르면 편성 서명이 갈려 렌더가 다시 청소했다(실측 cid57:
    #   영상 15편에 29콜). 순서대로 누른 고객이 그대로 두 배를 냈다.
    assert labels == ["영상추출/분석", "대본생성", "영상대본MIX", "TTS음성", "고품질 자막제거",
                      "장면꾸미기", "썸네일", "제목·태그", "완성본", "SNS 예약"], labels
...
def test_orb_to_panel_mapping_v6():
    ...
    # 2026-09-01 TTS(패널2)를 자막제거(패널1) 앞으로 — 표시 순서만 바뀌고 **패널 번호는 그대로**다.
    #   DB에 저장되는 step은 패널 인덱스라 기존 작업의 저장값이 그대로 유효하다(마이그레이션 없음).
    assert mapping == [0, 8, 7, 2, 1, 3, 4, 5, 6, 9], mapping
