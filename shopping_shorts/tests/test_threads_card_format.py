...
def test_조회수당댓글은_조회수가_있을때만():
    ...
    block = _card_block()
    # ★라벨 문구가 아니라 **지표 키**로 찾는다(2026-08-24). 지표 이름이 플랫폼별로
    #   갈리면서(METRIC_LABELS) 카드는 '조회수당댓글' 리터럴 대신 _mlabel('density')를
    #   쓴다 — 문구로 찾으면 이름을 고칠 때마다 이 검사가 깨진다. 지키려는 것은
    #   **조건부로 그리는가**이지 이름이 아니다.
    i = block.find("_mlabel('density')")
    assert i != -1, "density 지표 줄을 못 찾음"
    seg = block[max(0, i - 200):i]
    assert "i.views?" in seg, (
        "density를 조건 없이 그린다 — 조회수 없는 플랫폼(쓰레드)에서 늘 0%가 뜬다")
