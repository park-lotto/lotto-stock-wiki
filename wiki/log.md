...
**신규 파일**:
- `wiki/외부인사이트/index.md` — 소스 레지스트리 + 최근 핵심 인사이트 요약
...
- `wiki/L5_섹터/반도체/테이블.md` — 일별 활성 로그 5/7 ~ 5/25 추가
...
**수정 파일**:
- `wiki/L5_섹터/index.md` — 12개 섹터 전체 온도 현황표 + 자동반영 프로토콜 + 섹터 폴더 목록 전면 재편
...
- `wiki/L5_섹터/조선/테이블.md` (수정) — 12개 서브섹터 + 기술드라이버 × 서브섹터 연계 표 추가
...
- `wiki/L5_섹터/반도체/테이블.md` (수정) — 기술드라이버 × 서브섹터 연계 표 추가
...
- wiki/L2_미국시장/index.md — 재설계, 자동 업데이트 연동
