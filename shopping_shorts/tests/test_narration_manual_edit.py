...
"""
from shopping_shorts.edit_plan import (
    enforce_script_order, enforce_scripted_narration, script_sentences)
...
# ── 문장 분리기가 명사 안의 '요'에서 자르던 것 (2026-08-25 고객신고 #6) ────────
# ★라이브 실사고(job dce999992192): "소금도 필요 없이 딱 한 스푼 넣고 섞기만 해서 더
#   놀랐어요."가 beat2="소금도 필요" / beat3="없이 딱 한 스푼…" 두 칸으로 쪼개졌다.
#   enforce_script_order의 규약은 "문장은 쪼개지 않는다"인데, 분리기가 이미 잘못 자른
#   조각을 받으니 규약을 지켜도 소용이 없었다.
#   08-20 주석은 "작게 잘리는 쪽은 피해가 없다"고 봤지만, 그 뒤 생긴 순서 재배분이
#   조각을 각각 다른 칸에 넣으면서 실제 피해가 났다.


def test_명사_안의_요에서_문장을_자르지_않는다():
    """★실사고 문장 그대로. 필요·중요·주요는 종결어미가 아니다."""
    assert script_sentences("소금도 필요 없이 딱 한 스푼 넣고 섞기만 해서 더 놀랐어요.") == \
        ["소금도 필요 없이 딱 한 스푼 넣고 섞기만 해서 더 놀랐어요."]
    for s in ("이 제품 중요 부분을 보세요.", "주요 기능이 다 들어있어요.",
              "수요 예측이 빗나갔어요.", "동요 부르는 아이들이 귀여워요."):
        assert len(script_sentences(s)) == 1, f"'{s}' 가 중간에서 잘렸다"


def test_진짜_종결어미_요죠는_여전히_자른다():
    """★회귀 방지. 2026-08-20에 이 규칙을 넣은 이유(마침표 없는 구어체)는 그대로다.

    ⚠️어미 종류가 생각보다 많다 — '거든요'(앞 음절 '든')를 빠뜨렸다가
      08-20 테스트(test_scripted_narration_invariant)가 105자 덩이로 잡아냈다.
      아래는 실제 대본체에서 흔한 종결형을 모아 실측으로 확정한 것."""
    for s in ("그래서 샀어요 진짜 좋아요",
              "이거 좋더라구요 한번 써보세요",
              "맛있네요 또 사고 싶어요",
              "핵심이죠 확인해보세요",
              "진짜 스트레스였거든요 그래서 찾아봤어요",     # 거든요
              "이 제품이라는 거예요 한번 보세요",            # 거예요
              "저도 해봤더라고요 정말 편해요",               # 더라고요
              "쓰려고요 그래서 샀어요"):                     # 려고요
        assert len(script_sentences(s)) == 2, f"'{s}' 가 안 잘렸다"
