...
BRAND_STYLE_PRESETS = {
    ...
    "clay": (
        "[디자인 지침] 첨부된 소스 이미지들(Clay.com 실제 브랜드 3D 클레이메이션 일러스트)을 "
        "그대로 참고해서 재현해라 — 손으로 빚은 듯한 3D 클레이(점토) 스타일 입체 아이콘/일러스트를 "
        "핵심 비주얼로 써라(평면 벡터 아이콘 금지, 반드시 3D 클레이 질감). 배경은 크림빛 화이트"
        "(#fffaf0). 채도 높은 단색 카드를 섹션마다 순환해서 써라 — 핑크(#ff4d8b), 진한 틸/그린"
        "(#1a3a3a 또는 #0a5c3a), 라벤더(#b8a4ed), 피치/오렌지(#ffb084), 오커/머스타드(#e8b94a) "
        "중 매번 다른 색을 골라 카드 배경 전체를 채워라(같은 색 연속 금지). 헤드라인 폰트는 굵기 "
        "500 정도의 둥글둥글한 산세리프(굵게 bold 700 금지), 음수 자간. 카드 모서리는 큼직하게 "
        "둥글리고(16~24px), 그림자는 거의 쓰지 말고 색 대비로만 입체감을 내라. 전체적으로 따뜻하고 "
        "장난기 있는 느낌으로, 진지한 금융 대시보드가 아니라 친근한 SaaS 마케팅 페이지처럼 구성해라."
    ),
}
