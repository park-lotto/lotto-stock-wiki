...
## 🙋 이 세션은 여기서 손 뗌 — 다른 세션이 이어받아도 됩니다
이 세션은 지금부터 **레퍼런스 랭킹(계정등록 기반) 샤오홍슈 탭** 쪽으로 갑니다(별도 설계문서
`docs/superpowers/specs/2026-07-29-샤오홍슈-레퍼런스채널-design.md`) — **해외HOT(키워드검색 발굴)
쪽은 안 건드립니다.** 아래 내용은 전부 유효한 미완 상태 그대로입니다:
- **샤오홍슈 키워드검색 크롤러(Phase 1, `playwright_crawl.py`) 자체가 아직 코드로 없음** — 오늘
  검증한 건 일회성 테스트 스크립트뿐, 실제 모듈/파이프라인 연결은 미착수.
- **TikTok 캡차 미해결** — 아래 "⏭ TikTok" 절 참고.
- **재사용 가능 자산**: 로그인 세션 파일 `/home/ubuntu/rednote_session.json`(600권한)은 레퍼런스
  랭킹 작업에서도 그대로 쓰고 있음 — 만료돼서 재로그인하게 되면 두 작업 모두에 영향 있으니 알아둘 것.
  크롤러 코드 작성 시 `xiaohongshu_playwright.py`(레퍼런스 랭킹용, 프로필페이지 크롤)가 먼저 생길
  수 있음 — 세션 로드 부분은 그 모듈에서 패턴 참고 가능(단, 검색페이지 크롤은 다른 URL/셀렉터 필요).
