"""메인 로고·탭 제목 브랜드 = 숏템메이커 (사장님 2026-07-21 지시).

마케팅 대문(app.py _BRAND)은 이미 숏템메이커인데 내부 툴 페이지의 로고(사이드바)와
브라우저 탭 제목이 옛 '쇼핑쇼츠'로 남아 있었다. 고객 노출 브랜드를 숏템메이커로 통일한다.
...
def test_sidebar_logo_is_shottembox():
    ...
    assert "숏템메이커" in js, "사이드바 메인 로고가 숏템메이커가 아니다"
...
def test_page_titles_use_shottembox_not_old_brand():
    for name in _TITLE_PAGES:
        ...
        assert "숏템메이커" in head, f"{name} 탭 제목에 숏템메이커가 없다"
