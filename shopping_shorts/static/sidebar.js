...
(function () {
  ...
  //   - has_dead  → 교체 안내 팝업, 하루 1회
  //   - few_keys  → 예비 키 당부 팝업, 3일에 1회(키를 아예 안 낸 회원에겐 서버가 false를 준다)
  //   - 설정 화면에선 안 띄운다 — 거기엔 키마다 이유가 이미 펼쳐져 있다.
    ...
    _f("/api/settings/gemini_health").then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) {
        ...
        if (h.has_dead) {
          ...
          var lines = dead.map(function (k) { return "• " + (k.label || "키") + "\n   " + (k.title || ""); }).join("\n");
          opts = { icon: "⚠️", title: "등록하신 제미니 키 " + dead.length + "개가 멈췄어요",
                   body: lines + "\n\n" + (dead[0].fix || "") + "\n\n" + (h.tip || ""),
                   hideContact: true, link: "/settings#keys", linkText: "🔑 키 교체하러 가기", closeLabel: "나중에" };
          try { localStorage.setItem(_GKH_DEAD_KEY, today); } catch (e) {}
        } else if (h.few_keys) {
          var last = 0;
          try { last = parseInt(localStorage.getItem(_GKH_FEW_KEY) || "0", 10) || 0; } catch (e) {}
          if (Date.now() - last < 3 * 86400000) return;
          opts = { icon: "🔑", title: "제미니 키, 예비로 " + (h.recommended || 3) + "개 등록해 두세요",
                   body: "지금 쓸 수 있는 키 " + (h.n_usable || 0) + "개\n\n" + (h.tip || ""),
                   hideContact: true, link: "/settings#keys", linkText: "🔑 키 추가하러 가기", closeLabel: "나중에" };
          try { localStorage.setItem(_GKH_FEW_KEY, String(Date.now())); } catch (e) {}
        }
        if (!opts) return;
        // ★결제 안내 팝업(_payPrompt)과 같은 틀을 쓴다 — 그게 떠 있으면 덮지 않고 닫힐 때까지 기다린다.
        var tries = 0;
        (function show() {
          var ex = document.getElementById("ss-pw-modal");
          if (ex && ex.style.display !== "none" && tries++ < 60) { setTimeout(show, 2000); return; }
          _pwModal(opts);
        })();
      }).catch(function () {});
  }
  ...
  if (document.readyState === "loading") {
    ...
    document.addEventListener("DOMContentLoaded", function () { setTimeout(initGeminiKeyHealth, 1500); });
  } else {
    mount();
    __ssPaintTheme();
    mountWorks();
    initPaywall();
    initSignupAlert();
    setTimeout(initGeminiKeyHealth, 1500);
  }
})();
