...
ask 칸이 이 자리다. 인물(친구·언니·엄마·지인·사장님)은 이야기를 사게 하는 장치이므로
자연스럽게 등장시켜라. 이 흐름에 대화가 안 어울리면 빈칸으로 둬도 된다.
...
■ beats — 겪은 일을 before/after로
  ...
  from_pain  이 칸이 어느 특징에서 나왔는지 적어라(재료의 이름이나 '이게 없을 때' 문장).
칸 수를 채우려 하지 마라. 1개여도 2개여도 된다 — **억지로 늘리면 안 쓴 것만 못하다.**
...
불편·상황·인물·수치·효능은 **화면에 없어도 쓸 수 있다.** 그 제품을 쓰는 사람이라면
누구나 겪을 법한 일이면 된다. 사용자가 나중에 고치면 되니 **재미있게, 구체적으로** 써라.
  O 겨우 짜서 바르려는데 손가락 사이로 다 흘러내리고      (화면에 없어도 공감된다)
  O 지인이 다이소 매니저인데 이건 들어오자마자 없어진대요   (이야기를 사는 장치다)
  O 한 통이면 두 달은 쓴다는데                          (수치로 실감을 준다)
...
def _tok(t):
    """낱말 단위 — 글자 대조는 표현을 풀고 나면 쓸 수 없다."""
    return set(re.findall(r"[가-힣]{2,}", t or ""))


def audit(lines, o, feats):
    """★검사는 규칙과 같은 기준이어야 한다(0순위-B).
    표현을 자유롭게 푼 뒤(2026-09-22)에는 from_pain을 **글자로 대조하면 안 된다** —
    모델이 자기 말로 적으므로 멀쩡한 칸이 '근거없음'으로 잡힌다(실측 2건).
    이제는 **어느 특징에서 나왔나**만 본다: 특징 이름이나 pain과 낱말이 겹치면 통과."""
    pains = [(f.get("pain") or "").strip() for f in feats if (f.get("pain") or "").strip()]
    keys = [_tok(f.get("name", "") + " " + f.get("pain", "") + " " + f.get("claim", ""))
            for f in feats]
    bs = o.get("beats") or []
    ung = []
    for b in bs:
        fp = _tok(b.get("from_pain") or "")
        if not fp or not any(fp & k for k in keys):
