- 2026-07-05 — [구현/배포] **인사이트 페이지 리디자인 완성 + 이벤트캘린더 실데이터 파이프라인 복구**(브랜치 feat/briefing-engine): 세션F 미완이던 라이트테마+v5 1단레이아웃 이식 마무리(subagent-driven-development, Opus 구현+리뷰), 이후 사용자 실사용 피드백 다수 라운드 반영(브랜드아이콘 전면화, 시그널/이벤트/리포트/기록 "1행+더보기" 통일, 가독성 개선). **핵심 버그 발견**: "다가오는 이벤트" 0건이던 원인 = fetch_sector_calendar.py가 프리뷰모델(gemini-3-flash-preview, 무료쿼터 계정무관 즉시소진) 하드코딩 + 검색기간 리터럴날짜 하드코딩(이미 지난기간 검색) 이중버그 — 안정판+롤링윈도우로 수정 후 재수집 성공(111건). calendar_ingest의 "null"문자열 asset버그, calendar_build의 부분날짜(2026-07-XX) 파싱크래시도 추가발견·수정. 부수로 시그널 기사중복(태그차이 dedup누락)·리포트제목"parsed"버그도 수정, 인포그래픽 브랜드스타일(claude/clay) 선택버튼 신설. 상세=NEXT_SESSION.md 세션J, `.superpowers/sdd/progress.md` ledger(Task1~23).
- 2026-07-05 — [구현/배포] **크롤링 인제스트 자동점검·자동수정 파이프라인(v2) 완료**: 채널단위 raw신선도 감지→claude -p 2단계(진단전용/수정)→pytest게이트→배포(로컬 git/원격크롤러 파일교체, SSH불필요)→헬스체크+자동롤백→텔레그램 보고. daily_verify.py(v1) 대체, 서버 크론 등록(`50 8,12,15,18,21 --slot`+`45 21 --daily-summary`). subagent-driven-development로 10태스크 구현, 리뷰 과정에서 실버그 5개 발견·수정(스펙값 임의변경·wiki디렉터리 크래시·알림summary누락·백업복원누락·remote_crawler 변경범위 신뢰문제). **실전 dry-run 검증**: 11채널 이상감지→전부 에스컬레이션, 그 중 2개는 실제 크롤러코드 수정 시도까지 갔으나 안전게이트 걸려 전부 자동롤백 확인(라이브파일 diff 0, 서비스 정상 유지). 안전장치 실전 작동 실증. 상세=NEXT_SESSION.md 세션I.
- 2026-07-05 — [버그수정/긴급] **서버 인제스트 크래시 수정(key_vault msvcrt)**: 자동화 파이프라인 브레인스토밍 중 실서버(라이브 대시보드가 실제로 읽는 `/home/ubuntu/lotto-stock-wiki`)의 텔레그램·뉴스 인제스트가 최소 07-03부터 크래시 중이었던 것 발견 — `pipeline/atoms/key_vault.py`의 `_FileLock`이 Windows 전용 `msvcrt`만 써서 리눅스에서 키 로테이션마다 `ModuleNotFoundError`. 오늘 서버 atoms.db가 텔레그램 9개/뉴스 0개(로컬은 256개/102개)로 실제 데이터 기아 상태였음. `sys.platform` 분기로 POSIX는 `fcntl.flock` 사용하도록 수정, 테스트 추가(로컬15개 통과+서버 재검증15개 통과), main에 직접 커밋(2c6c05b0, 임시 워크트리로 feat/briefing-engine 안건드리고 체리픽)+서버 배포(stash-pull-pop, 충돌없음). 이 버그 관련 테스트가 07-03부터 daily_verify 텔레카드에 "pytest 실패"로 매일 찍혔었는데 v1은 발견만 하고 안 고쳐서 방치됐던 것 — 지금 설계중인 자동수정 파이프라인이 딱 이런 사례를 막기 위한 것. 로컬/서버 atoms.db가 완전히 독립된 두 사본이며 git으로 추적되는 문제는 별도 미해결(다음에 정리 필요).
- 2026-07-05 — [버그수정] **텔레그램 news_relay 4채널 0원자 버그 수정**: `telegram_channels.json`에 등록만 되고 `telegram_questionnaire.py`엔 구현 없던 `news_relay` 타입(주식픽·실시간속보단독뉴스·실시간주식뉴스·그로쓰리서치특징주) 질문지+원자변환 추가, Gemini 응답 JSON 뒤 여분문자 파싱실패 폴백 추가. 실채널 검증(29/31/6개 원자, 이전 0개). **미해결**: 실시간주식뉴스·그로쓰리서치특징주 두 채널은 원격 크롤봇(Lightsail) 계정이 07-03 이후 텔레그램 새 메시지를 못 받아옴(플랫폼 리드제한 추정, 코드문제 아님). 별개로 fablize 훅이 python3→윈도우스토어 스텁 잡던 버그도 발견, PATH 재정렬(재시작 후 확인 필요). 상세=NEXT_SESSION.md 세션H.
...
## 2026-07-05 (세션K 밤) — YT 레퍼런스 창고 대확장 + 텔레 2채널 진단
- /yt/refs 확장(커밋 dcf9c7b6): AI자연어검색·⭐배수반영·스토리라인→대본(Opus)·주장기반 리서치(근거검증). 시황브리핑 주말heartbeat 끄기(fe56ad0a).
- 텔레 2채널(realtime_stock_news·rocket_news1) 크롤중단 = shadow limit 아니라 채널 자체 7/3 게시중단(공개프리뷰 확정). 세션H 오진 정정.
