...
(async()=>{
 ...
 const before=await p.evaluate(()=>{const el=window.__card;
   ...
   return {tf:getComputedStyle(n).transform, img:el.querySelector('.precision-media').src.slice(-12),
           anims:document.getAnimations().length, ct:document.getAnimations().map(a=>a.currentTime)};});
 console.log('BEFORE SHOT', JSON.stringify(before));
 await p.screenshot({path:'out/_t.png',clip:{x:0,y:0,width:304,height:540}});
 const after=await p.evaluate(()=>{const el=window.__card;
   const n=el.querySelector('.precision-text.center');
   return {tf:getComputedStyle(n).transform, img:el.querySelector('.precision-media').src.slice(-12),
           anims:document.getAnimations().length, ct:document.getAnimations().map(a=>a.currentTime)};});
