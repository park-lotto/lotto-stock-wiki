...
def attribute_mover(mover: dict, days: int = 3, query_fn=None, related_fn=None,
                    min_graph_strength: int = 3) -> dict:
    """직접 원자 → 없으면 그래프-홉(같은 섹터 관련자산의 '강한' 호재) → 없으면 미귀속(침묵 금지).

    min_graph_strength: 그래프-홉 연계는 관련자산 원자의 strength_score가 이 값 이상일 때만.
    약한 섹터 동조를 걸러 '진짜 미귀속 알파'가 묻히지 않게 한다(직접 귀속엔 임계 없음).
    """
    if query_fn is None:
    ...
    # 그래프-홉 폴백: 같은 섹터 관련자산의 '강한' 호재 원자를 근거로 연계 귀속
    # related_fn은 종목명을 받아 그래프 2홉(종목→큰섹터→관련자산)으로 관련자산을 반환한다.
    if related_fn is not None:
        for rel in related_fn(name) or []:
            if rel == name:
                continue
            r_hits = [a for a in _bullish_hits(query_fn(asset=rel, days=days, active_only=True))
                      if (a.get("strength_score") or 1) >= min_graph_strength]
            if r_hits:
                rt = r_hits[0]
...
def scan_movers(movers: list, days: int = 3, query_fn=None, related_fn=None,
                min_graph_strength: int = 3) -> list:
    """모든 강세 종목을 귀속 판정. 입력 종목은 하나도 누락하지 않는다(침묵 금지)."""
    return [attribute_mover(m, days=days, query_fn=query_fn, related_fn=related_fn,
                            min_graph_strength=min_graph_strength) for m in movers]
...
def scan_heatmap(top_n: int = 5, days: int = 3, min_rate: float = 3.0,
                 min_graph_strength: int = 3) -> dict:
    ...
    results = rank_results(scan_movers(movers, days=days, related_fn=_edges.related_assets,
                                       min_graph_strength=min_graph_strength))
