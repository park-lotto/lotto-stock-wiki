...
### 1) 스모크 테스트 — 18개 키 전수 인증 확인 (`scripts/smoke_test_key_vault.py`)

18개 키(general 4 + ingest 5 + embed 6 + briefing 3) 각각에 실제 1콜씩 전송한 결과:

- **최종(공식) 실행**: `총 18개 키 확인: OK 16개, RATE_LIMITED 2개, 실패 0개`
  (`ingest#1`, `ingest#2`가 `429 RESOURCE_EXHAUSTED` — 키 자체는 정상 인증되었고,
  `main` 브랜치에서 이미 돌고 있는 동시 프로덕션 프로세스가 같은 `ingest` 그룹 키를
  공유해서 쓰기 때문에 발생한 일시적 쿼터 경합으로 판단. 인증 실패(`AUTH_FAIL`,
  예: `API_KEY_INVALID`/`PERMISSION_DENIED`)는 18개 키 중 단 하나도 없었음.)
- exit code: `0`
- 상세 결과와 원본 stdout: `.superpowers/sdd/task-12-report.md`

### 2) 부하 테스트 — embed 풀(6키) 피크 물량 흡수 확인 (`scripts/loadtest_embed_pool.py --n 1200`)

2026-07-01 실측 피크(임베딩 호출 1,171건/일)보다 약간 많은 1,200건을 embed 그룹
6개 키에 실제로 흘려서 측정:

```
시작 시점 살아있는 embed 키: 6/6
총 1200건, 실패 0건, 소요 553.6s (9.2분)
종료 시점 살아있는 embed 키: 6/6
```

- **실패**: 0/1200
- **소요 시간**: 553.6초 (9.2분)
- **키 소진**: 없음 — 시작/종료 시점 모두 6/6 생존
- `embed` 그룹은 다른 그룹과 달리 오늘 마이그레이션으로 신설된 그룹이라
  `main` 브랜치의 동시 프로세스가 아직 이 키들을 쓰지 않으므로, 이 측정은
  외부 트래픽에 오염되지 않은 embed 풀 자체의 순수 처리 능력에 가깝다.
- 상세 결과(정확한 실행 커맨드, 전체 원본 stdout, 스크립트 마지막 줄에서
  발생했던 Windows `cp949` 인코딩 크래시와 그 수정 내역 포함):
  `.superpowers/sdd/task-13-report.md`

**결론**: embed 그룹 6키는 오늘 실측 피크치를 실패 없이, 키 소진 없이 흡수한다.
현재 설계(ingest 5 / embed 6 / general 4 / briefing 3)의 재조정은 이 시점 기준
불필요.
